...
def run(args, parser):
    if args.config_file:
        with open(args.config_file) as f:
            ...
            # doesn't know how to insert functions in yaml yet
            experiment_name = next(iter(experiments))
            experiments[experiment_name]["config"]["callbacks"] = {
                    "on_episode_start": ray.tune.function(cm.on_episode_start),
                    "on_episode_step": ray.tune.function(cm.on_episode_step),
                    "on_episode_end": ray.tune.function(cm.on_episode_end),
                    "on_sample_end": ray.tune.function(cm.on_sample_end),
                    "on_train_result": ray.tune.function(cm.on_train_result),
                    }
    else:
...
if __name__ == "__main__":
    ...
    # config for v4 
    # env_config = {
    #   "env": "Insertion_Real_v4", 
    #   "renders": "False", 
    #   "stepLimit": "True", 
    #   "random_box_pos": "False", 
    #   "random_peg_pose": "True"
    #   }
    # environment = el.env_creator(env_config)

    random.seed(123)

    #-- autodesk_rl --#
    register_env("ROBOTIC_ASSEMBLY", el.env_creator) # customized envs
    register_trainable("APEX_DDPG_HD", get_agent_class("contrib/APEX_DDPG_HD")) # customized algorithms: Dynamic Human Demostrations
    #-- autodesk_rl --#
